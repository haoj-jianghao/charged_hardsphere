...
!C    *******************************************************************
        ...
        DOUBLE PRECISION   TWOPI, B, RKX, RKY, RKZ, RKSQ, KAPP
        PARAMETER ( KMAX = 7, KSQMAX = 125 , TWOPI = 6.2831853 )
...
!C    ** LOOP OVER K-VECTORS. NOTE KX IS NON-NEGATIVE **
        ...
        DO 100 KX = 0, KMAX
           ...
           RKX = TWOPI * REAL ( KX ) / BOX

           DO 99 KY = -KMAX, KMAX

              RKY = TWOPI * REAL ( KY ) / BOX

              DO 98 KZ = -KMAX, KMAX

                 RKZ = TWOPI * REAL ( KZ ) / BOX
...
!        INCLUDE 'conf.inc'
        ...
        DOUBLE PRECISION, INTENT(IN) ::   RX(N), RY(N), RZ(N)!, KAPPA
        DOUBLE PRECISION, INTENT(OUT) ::    VR

        DOUBLE PRECISION ::   RXI, RYI, RZI, RXJ, RYJ, RZJ, RXIJ, RYIJ, RZIJ
        DOUBLE PRECISION ::   RIJSQ, RIJ, KRIJ, ERFC, QI
...
!C    *******************************************************************
        ...
        INTEGER     MAXK ,  N
        PARAMETER ( MAXK = 2000, N = 2048)

        DOUBLE PRECISION, INTENT(IN)::   RX(N), RY(N), RZ(N)!,KAPPA  !, KVEC(MAXK),
        DOUBLE PRECISION, INTENT(out)::   VK
        INTEGER     TOTK

        INTEGER     KMAX, KX, KY, KZ, I, KSQMAX, KSQ
        DOUBLE PRECISION    TWOPI, FACTOR, RSQPI
        DOUBLE PRECISION VD, VS
...
!C    ** CALCULATE KX, KY, KZ = 0 , -1 AND 1 EXPLICITLY **
        ...
        DO 10 I = 1, Nelec
           ...
           EIKX(I, 1) = CMPLX ( COS ( TWOPI * RX(I) /BOX ) ,  SIN ( TWOPI * RX(I) /BOX ) )
           EIKY(I, 1) = CMPLX ( COS ( TWOPI * RY(I) /BOX ) ,  SIN ( TWOPI * RY(I) /BOX ) )
           EIKZ(I, 1) = CMPLX ( COS ( TWOPI * RZ(I) /BOX ) ,  SIN ( TWOPI * RZ(I) /BOX ) )
