...
!c    NPART      : total numbero for particles
!c    TEMP       : temperature
!c    rho        : density
!c
...
!      INCLUDE 'conf.inc'
      ...
      OPEN (15, FILE='15an.dat',status='old')
      OPEN (25, FILE='25.dat',status='old')
	  OPEN (11, FILE='11.dat',status='old')
	  OPEN (22, FILE='ratio.txt')
	  OPEN (23, FILE='radial1.txt')
	  OPEN (24, FILE='radial2.txt')
	  OPEN (26, FILE='radial3.txt')
 
 
!c     ---read simulation data
!C      READ (15, *)
      READ (15, *) ibeg, Equil, Prod, Nsamp, Iseed, &
     &	         Dr, Ndispl, NPART, TEMP,  rho 
!C      READ (15, *)
!C      READ (15, *) Dr
!C      READ (15, *)
!c      READ (15, *) Ndispl
!c      READ (15, *)
!c      READ (15, *) NPART, TEMP, rho
!c     ---initialise and test random number generator

      ALLOCATE (X(NPART), Y(NPART), Z(NPART), Q(NPART),Q_scale(6,NPART+1))
      ALLOCATE (Xh(NPART), Yh(NPART), Zh(NPART))
      ALLOCATE (Xi(NPART), Yi(NPART), Zi(NPART))
      ...
      IF (NPART.GT.NPMax) THEN
         ...
         STOP
      END IF
      BOX = (NPART/rho)**(1.D0/3.D0) * 0.80599575  !correction for r_star = pi/6*rho*sig^3 correct for the pi/6
      HBOX = BOX/2
